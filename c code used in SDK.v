...
for(i = 1; i < NUMTEST + 1; i++)
	for(j = 1; j < NUMIN + 1; j++)
	{
		...
		TxBufferPtr[Index] = 0.03125*i*16394;
		Index ++;
	}
...
int seperate()
{
	for(i = 1; i <= NUMTEST; i++)
		for(j = 1; j <= NUMIN; j++)
		{
			Test_input1[i][j] = Test_input[i][j]*16384;
		}
	for (j = 1; j <= NUMHID; j++)
	for (i = 1; i <= NUMIN; i++)

		{
			WeightIH1[i][j] = WeightIH[i][j] * 16384;
	...
	for (k = 1; k <= NUMOUT; k++)
		for (j = 1; j <= NUMHID; j++)
		{
			...
			WeightHO1[j][k] = WeightHO[j][k] * 16384;
